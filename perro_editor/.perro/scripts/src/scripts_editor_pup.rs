...
const __PERRO_SOURCE_FILE: &str = "scripts_editor.pup";
...
impl Script for ScriptsEditorPupScript {
    fn init(&mut self, api: &mut ScriptApi<'_>) {
        api.connect_signal_id(16479613592036736479u64, self.id, 16479613592036736479u64);
        api.connect_signal_id(10103917068867085514u64, self.id, 10103917068867085514u64);
    }
...
static VAR_GET_TABLE: phf::Map<u64, fn(&ScriptsEditorPupScript) -> Option<Value>> =
    phf::phf_map! {

    };

static VAR_SET_TABLE: phf::Map<u64, fn(&mut ScriptsEditorPupScript, Value) -> Option<()>> =
    phf::phf_map! {

    };

static VAR_APPLY_TABLE: phf::Map<u64, fn(&mut ScriptsEditorPupScript, &Value)> =
    phf::phf_map! {
...
> = phf::phf_map! {
        16479613592036736479u64 => | script: &mut ScriptsEditorPupScript, params: &[Value], api: &mut ScriptApi<'_>| {
            script.__t_two_d_Pressed(api);
        },
        10103917068867085514u64 => | script: &mut ScriptsEditorPupScript, params: &[Value], api: &mut ScriptApi<'_>| {
            script.__t_file_Pressed(api);
        },

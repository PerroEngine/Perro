...
pub fn compile_project_bundle(project_root: &Path) -> Result<(), CompilerError> {
    ...
    generate_static_materials(project_root)?;
...
fn generate_static_scenes(project_root: &Path) -> Result<(), CompilerError> {
    ...
    let scenes_src = format!(
        "// Auto-generated by Perro Compiler. Do not edit.\n\
...
fn generate_static_materials(project_root: &Path) -> Result<(), CompilerError> {
    let res_dir = project_root.join("res");
    let static_dir = project_root.join(".perro").join("project").join("src").join("static");
    fs::create_dir_all(&static_dir)?;
    ...
    // Deduplicate identical material payloads so multiple source paths can reuse
    // one canonical static value.
    ...
    out.push_str("// Auto-generated by Perro Compiler. Do not edit.\n");
    ...
    fs::write(static_dir.join("mod.rs"), "pub mod scenes;\npub mod materials;\n")?;
    Ok(())
}

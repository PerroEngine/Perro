...
fn write_scripts_lib(scripts_src: &Path, copied: &[String]) -> Result<(), CompilerError> {
    ...
    out.push_str("use perro_runtime::{Runtime, RuntimeResourceApi};\n");
    ...
    out.push_str("pub static SCRIPT_REGISTRY: &[(&str, ScriptConstructor<Runtime, RuntimeResourceApi>)] = &[\n");
    for rel in copied {
        let module = module_name_from_rel(rel);
        out.push_str(&format!(
            "    (\"res://{rel}\", {module}::perro_create_script as ScriptConstructor<Runtime, RuntimeResourceApi>),\n"
        ));
    }
    out.push_str("];\n");
    out.push_str(
...
pub extern \"C\" fn perro_script_registry_get(\n\
    ...
    len_out: *mut usize,\n\
    ctor_out: *mut ScriptConstructor<Runtime, RuntimeResourceApi>,\n\
) -> bool {\n\
...
impl<RT: RuntimeAPI + ?Sized, RS: perro_resource_context::api::ResourceAPI + ?Sized> ScriptBehavior<RT, RS> for {script_ty} {{
...
fn generate_call_method_body(methods: &[ScriptMethod]) -> String {
    ...
    for method in methods {
        ...
        let call = if method.takes_raw_params {
            format!("self.{}(ctx, res, self_id, params)", method.name)
        } else if method.params.is_empty() {
            format!("self.{}(ctx, res, self_id)", method.name)
        } else {
            let args = method
                .params
                .iter()
                ...
                .collect::<Vec<_>>()
                .join(", ");
            format!("self.{}(ctx, res, self_id, {args})", method.name)
        };

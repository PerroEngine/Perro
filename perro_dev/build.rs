...
"#
    ...
    println!("cargo:rustc-env=EMBEDDED_ICON_MODULE={}", embedded_icon_module.display());
}
...
fn setup_linux_desktop(icon_path: &Path, log_path: &Path, name: &str, version: &str) {
    ...
    log(log_path, &format!("✔ Copied icon to {}", icon_dest.display()));
    ...
    let binary_name = std::env::var("CARGO_BIN_NAME")
        .unwrap_or_else(|_| "PerroDevRuntime".to_string());
...
"#,
    ...
    log(log_path, &format!("✔ Created Linux desktop file: {}", desktop_path.display()));
    ...
    log(log_path, &format!("✔ Created binary desktop file: {}", binary_desktop_path.display()));
    ...
    if let Ok(home) = std::env::var("HOME") {
        ...
        if fs::create_dir_all(&apps_dir).is_ok() {
            ...
            if fs::copy(&desktop_path, &system_desktop_path).is_ok() {
                log(log_path, &format!("✔ Installed desktop file to: {}", system_desktop_path.display()));
            }
            
            let binary_system_desktop_path = apps_dir.join(format!("{}.desktop", binary_name_lower));
            if fs::copy(&binary_desktop_path, &binary_system_desktop_path).is_ok() {
                log(log_path, &format!("✔ Installed binary desktop file to: {}", binary_system_desktop_path.display()));
            }
    ...
    if let Ok(target_dir) = std::env::var("CARGO_TARGET_DIR") {
        ...
        if fs::write(&bin_desktop_path, &desktop_content).is_ok() {
            log(log_path, &format!("✔ Created desktop file next to binary: {}", bin_desktop_path.display()));
        }
...
fn install_icon_to_system(icon_path: &Path, home: &str, icon_name: &str, log_path: &Path) {
    ...
    let img = match ImageReader::open(icon_path) {
        Ok(reader) => match reader.decode() {
            Ok(img) => img,
            Err(e) => {
                log(log_path, &format!("⚠ Failed to decode icon: {}, skipping system installation", e));
                return;
            }
        },
        Err(e) => {
            log(log_path, &format!("⚠ Failed to open icon: {}, skipping system installation", e));
            return;
        }
    };
    ...
    for size in sizes.iter() {
        let icons_dir = PathBuf::from(home).join(format!(".local/share/icons/hicolor/{}x{}/apps", size, size));
        ...
        if resized.save(&icon_file).is_ok() {
            log(log_path, &format!("✔ Installed {}x{} icon to: {}", size, size, icon_file.display()));
        }
    }
            ...
            match output {
                Ok(result) => {
                    if result.status.success() {
                        log(log_path, &format!("✔ Registered icon with xdg-icon-resource: {}", icon_name));
                    }
                }
                Err(e) => {
                    log(log_path, &format!("⚠ xdg-icon-resource failed: {} (this is OK, manual install still works)", e));
                }
            }
        }
    }
...
fn create_appimage(icon_path: &Path, log_path: &Path, name: &str, version: &str) {
    ...
    let target_dir = std::env::var("CARGO_TARGET_DIR")
        .unwrap_or_else(|_| PathBuf::from("..").join("target").to_string_lossy().to_string());

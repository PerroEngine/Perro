...
fn insert_static_scene_nodes(runtime: &mut Runtime, scene: &'static StaticScene) -> Result<(), String> {
    ...
    for static_node in scene.nodes {
        ...
        let Some(parent_key) = static_node.parent else {
            continue;
        };
        let parent_id = *key_to_id.get(parent_key.0).ok_or_else(|| {
            format!(
                "parent node key `{}` not found while linking child `{}`",
                parent_key.0, static_node.key.0
            )
        })?;
...
impl Runtime {
    fn attach_scene_scripts(&mut self) -> Result<(), String> {
        ...
        for (id, node) in self.nodes.iter() {
            if let Some(script) = node.script.as_deref() {
                script_nodes.push((id, script.to_string()));
            }
        }
    ...
    fn ensure_dynamic_script_registry_loaded(&mut self, _project_root: &Path) -> Result<(), String> {
        ...
        let library = unsafe {
            libloading::Library::new(&dylib_path)
                .map_err(|err| format!("failed to load scripts dylib `{}`: {err}", dylib_path.display()))?
        };
        ...
        unsafe {
            ...
            type RegistryGetFn =
                unsafe extern "C" fn(usize, *mut *const u8, *mut usize, *mut ScriptConstructor<Runtime>) -> bool;
            ...
            let registry_len = *library
                ...
                .map_err(|err| format!("missing `perro_script_registry_len` in scripts dylib: {err}"))?;
            let registry_get = *library
                .get::<RegistryGetFn>(b"perro_script_registry_get")
                .map_err(|err| format!("missing `perro_script_registry_get` in scripts dylib: {err}"))?;

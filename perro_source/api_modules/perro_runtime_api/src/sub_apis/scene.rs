...
pub trait SceneAPI {
    ...
    /// `scene_load_preloaded` without a wait. Runtimes that preload
    /// synchronously report every live handle as ready.
...
impl<'rt, R: SceneAPI + ?Sized> SceneModule<'rt, R> {
    ...
    /// Start a preload and get its handle immediately.
    ///
    /// The scene parses + prepares on a worker, like mesh/material/texture
    /// loads: check `preload_ready` before using the handle, or hand it to
    /// `load_preloaded` and accept a wait if the work has not landed yet.
    ...
    /// Whether a `preload` handle finished loading.
    ///
    /// ```ignore
    /// let next = ctx.Scene().preload("res://levels/2.scn")?;
    /// // later frames
    ...
    /// Whether a `preload` handle is still loading on a worker.

...
pub fn generate_static_localizations(
    ...
    let locales: Vec<(String, usize)> = headers
        .iter()
        .enumerate()
        .filter_map(|(i, name)| {
            if i == key_idx {
    ...
    struct LocaleRows {
        ...
        row_index_by_hash: HashMap<u64, usize>,
    }

    let mut per_locale: Vec<(String, LocaleRows)> = locales
        .iter()
        .map(|(locale, _)| {
            (
                locale.clone(),
    ...
    let mut key_rows: Vec<(u64, String)> = Vec::new();
    ...
    for row in reader.records() {
        ...
        if !key_hashes.contains_key(&hash) {
            key_hashes.insert(hash, key.to_string());
            key_rows.push((hash, key.to_string()));
        }
        ...
        for (locale_entry, (_, idx)) in per_locale.iter_mut().zip(locales.iter()) {
            ...
            if let Some(existing_index) = locale_rows.row_index_by_hash.get(&hash).copied()
                && locale_rows.rows[existing_index].1 != key
            {
                return Err(StaticPipelineError::SceneParse(format!(
                    "localization hash collision in `{}` for locale `{locale_name}` between keys `{}` and `{key}`",
                    localization.source_csv, locale_rows.rows[existing_index].1
                )));
            }
            if let Some(existing_index) = locale_rows.row_index_by_hash.get(&hash).copied()
                && locale_rows.rows[existing_index].1 == key
            {
                locale_rows.rows[existing_index].2 = value;
                continue;
            }
            if let Some(existing_key) = key_hashes.get(&hash)
                && existing_key != key
            {
                return Err(StaticPipelineError::SceneParse(format!(
                    "localization hash collision in `{}` for locale `{locale_name}` between keys `{existing_key}` and `{key}`",
                    localization.source_csv
                )));
            }
            let insert_index = locale_rows.rows.len();
            locale_rows.rows.push((hash, key.to_string(), value));
            locale_rows.row_index_by_hash.insert(hash, insert_index);
        }
    }
    ...
    for (locale, locale_rows) in &per_locale {
        ...
        for (hash, key, value) in &locale_rows.rows {
            let _ = writeln!(
                out,
                "        {} => Some(\"{}\"),",
                key_const_name(key, *hash),

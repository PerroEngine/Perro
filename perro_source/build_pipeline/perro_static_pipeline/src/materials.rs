...
use perro_render_bridge::{
    ...
    MAX_VERTEX_MODIFIERS, PixelSurfaceMaterial3D, StandardMaterial3D, ToonMaterial3D,
    UnlitMaterial3D, VertexAxis3D, VertexModifier3D, VertexModifierMask3D,
};
...
pub fn generate_static_materials(
    project_root: &Path,
    ...
    out.push_str(
        "use perro_render_bridge::{CustomMaterial3D, CustomMaterialImage3D, CustomMaterialLighting3D, CustomMaterialParam3D, CustomMaterialParamValue3D, HandDrawnMaterial3D, Material3D, PixelSurfaceMaterial3D, StandardMaterial3D, ToonMaterial3D, UnlitMaterial3D, VertexAxis3D, VertexModifier3D, VertexModifierMask3D};\nuse std::borrow::Cow;\n\n",
    );
...
mod tests {
    ...
    use perro_render_bridge::{
        ...
        CustomMaterialParamValue3D, HandDrawnMaterial3D, Material3D, PixelSurfaceMaterial3D,
    ...
    #[test]
    fn material_codegen_emits_stylized_presets() {
        let hand = material_literal_to_code(&MaterialLiteral::HandDrawn(HandDrawnMaterial3D {
            hatch_scale: 40.0,
            ..Default::default()
        }));
        assert!(hand.contains("Material3D::HandDrawn"));
        assert!(hand.contains("hatch_scale:"));

        let pixel =
            material_literal_to_code(&MaterialLiteral::PixelSurface(PixelSurfaceMaterial3D {
                pixel_count: 48,
                color_levels: 6,
                ..Default::default()
            }));
        assert!(pixel.contains("Material3D::PixelSurface"));
        assert!(pixel.contains("pixel_count: 48"));
        assert!(pixel.contains("color_levels: 6"));
    }
    ...
    fn material_codegen_emits_custom_lighting_mode() {
        ...
        assert!(code.contains("surface: StandardMaterial3D {"));
        assert!(code.contains("alpha_mode: 2"));
        assert!(code.contains("double_sided: true"));
    }
...
"#,
            r#"
type = "hand_drawn"
base_color_factor = (0.9, 0.8, 0.6, 1.0)
band_count = 5
hatch_scale = 30.0
grain_strength = 0.1
"#,
...
vertex_modifiers = [
    { type = "wave", axis = "y", direction = (1.0, 0.0, 0.0), amplitude = 0.2, speed = 1.5 },
    { type = "pixel_snap", virtual_height = 32, strength = 1.0 }
]
"#,
            r#"
type = "custom"
...
"#,
    ...
    fn literal_to_material(literal: MaterialLiteral) -> Material3D {
        match literal {
            ...
            MaterialLiteral::HandDrawn(material) => Material3D::HandDrawn(material),
            MaterialLiteral::PixelSurface(material) => Material3D::PixelSurface(material),

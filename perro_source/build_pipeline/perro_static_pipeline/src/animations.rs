...
pub fn generate_static_animations(project_root: &Path) -> Result<(), StaticPipelineError> {
    ...
    ensure_unique_hashes(
        "animation",
        parsed.iter().map(|anim| anim.lookup_key.as_str()),
    )?;
...
fn optimize_animation_clip(
    mut clip: perro_animation::AnimationClip,
) -> perro_animation::AnimationClip {
...
fn sample_track_value_from_keys(
    keys: &[AnimationObjectKey],
    frame: u32,
...
"#;
    ...
    fn assert_track_sampling_equal(
        original: &[AnimationObjectKey],
        optimized: &[AnimationObjectKey],
    ) {
        let start = original
            .iter()
            .map(|k| k.frame)
    ...
    fn optimize_track_keys_preserves_samples_with_ease_curve() {
        ...
        let original = vec![
            key(
                ...
                AnimationInterpolation::Linear,
                AnimationEase::EaseInOut,
                AnimationTrackValue::Vec3([0.0, 10.0, -4.0]),
            ),
            key(
                25,
                AnimationInterpolation::Linear,
                AnimationEase::EaseInOut,
                AnimationTrackValue::Vec3([2.0, 6.0, -1.0]),
            ),
            key(
                50,
                AnimationInterpolation::Linear,
                AnimationEase::EaseInOut,
                AnimationTrackValue::Vec3([4.0, 2.0, 2.0]),
            ),
        ];
        track.keys = Cow::Owned(original.clone());
        optimize_animation_track_keys(&mut track);

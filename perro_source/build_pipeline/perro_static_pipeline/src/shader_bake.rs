...
#[derive(Default)]
pub(crate) struct ShaderBaker {
    context: Option<(wgpu::Device, wgpu::Queue)>,
    #[cfg(test)]
    device_creations: usize,
}
...
impl ShaderBaker {
    pub(crate) fn bake(&mut self, job: &ShaderBakeJob) -> Result<Vec<u8>, StaticPipelineError> {
        ...
        if self.context.is_none() {
            self.context = Some(pollster::block_on(create_bake_context())?);
            #[cfg(test)]
            {
                self.device_creations += 1;
            }
        }
        let (device, queue) = self.context.as_ref().expect("bake context initialized");
        let result = pollster::block_on(bake_shader_texture_async(job, device, queue));
        if result.is_err() {
            // A failed/lost device must not poison the next job or next build.
            self.context = None;
        }
        result
    }
...
impl ShaderBaker {
    pub(crate) fn bake(&mut self, _job: &ShaderBakeJob) -> Result<Vec<u8>, StaticPipelineError> {
...
async fn create_bake_context() -> Result<(wgpu::Device, wgpu::Queue), StaticPipelineError> {
...
async fn bake_shader_texture_async(
    job: &ShaderBakeJob,
    device: &wgpu::Device,
    queue: &wgpu::Queue,
...
mod tests {
    use super::{ShaderBakeJob, ShaderBaker};

    #[test]
    #[ignore = "requires GPU adapter"]
    fn bakes_wgsl_to_rgba_pixels() {
        let mut baker = ShaderBaker::default();
        let mut job = ShaderBakeJob {
...
"#
        ...
        let pixels = baker.bake(&job).expect("shader bakes");
        ...
        job.resolution = [4, 3];
        job.params[0][0] = 0.75;
        let second = baker.bake(&job).expect("second shader bakes");
        assert_eq!(second.len(), 48);
        assert!(second[2] > pixels[2]);
        assert_eq!(baker.device_creations, 1);
    }
}

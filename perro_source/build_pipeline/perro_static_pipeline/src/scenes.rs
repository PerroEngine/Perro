...
pub fn generate_static_scenes(
    ...
    let lookup_entries = scene_paths
        .iter()
        .enumerate()
        .map(|(index, p)| {
            (
                perro_ids::string_to_u64(p),
                format!("SCENE_HASH_{index}"),
                format!("&SCENE_{}", sanitize_ident(p)),
            )
        })
        .collect::<Vec<_>>();
...
use perro_scene::{{NodeType, SceneFieldName, SceneNodeDataBase, SceneObjectField, SceneValue, SceneValueKey, SceneNodeData, SceneNodeEntry, Scene, SceneKey}};\n\
use std::borrow::Cow;\n\n\
{shared_consts}\
{scene_defs}\n\
{lookup}",
...
fn emit_node_data_consts(
    ...
    *counter += 1;
    ...
    emit_static_node_type(node_type.name())?;
    ...
    let _ = writeln!(
        out,
        "const {data_name}: SceneNodeData = SceneNodeData {{ node_type: NodeType::{node_type}, fields: Cow::Borrowed({fields_ref}), base: {base_ref} }};"
    );
    Ok(data_name)
}

fn emit_static_node_type(ty: &str) -> Result<NodeType, StaticPipelineError> {
    match ty {
...
mod tests {
    ...
    fn static_node_type_supports_scene_aliases() {
        let aliases = [
            ("UiScroll", NodeType::UiScrollContainer),
            ("UiHBox", NodeType::UiHLayout),
            ("UiVBox", NodeType::UiVLayout),
        ];

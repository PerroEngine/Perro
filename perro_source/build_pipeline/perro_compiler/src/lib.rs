...
pub fn sync_scripts(project_root: &Path) -> Result<Vec<String>, CompilerError> {
    ...
    let _ = write_scripts_lib(&scripts_src, &copied)?;
    Ok(copied)
}
...
fn write_scripts_lib(scripts_src: &Path, copied: &[String]) -> Result<bool, CompilerError> {
...
impl<RT: RuntimeAPI + ?Sized, RS: perro_api::resource_context::api::ResourceAPI + ?Sized, IP: perro_api::input::InputAPI + ?Sized> ScriptBehavior<RT, RS, IP> for {script_ty} {{
...
#[allow(improper_ctypes_definitions)]
pub extern "C" fn perro_create_script() -> *mut dyn ScriptBehavior<perro_runtime::Runtime, perro_runtime::RuntimeResourceApi, perro_runtime::RuntimeInputApi> {{
    let script: Box<dyn ScriptBehavior<perro_runtime::Runtime, perro_runtime::RuntimeResourceApi, perro_runtime::RuntimeInputApi>> =

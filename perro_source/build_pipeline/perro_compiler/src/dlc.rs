...
fn write_dlc_pack_manifest(
    ...
    let perro_ids_path = normalize_toml_path(
        &engine_root
            .join("perro_source")
            .join("core")
            .join("perro_ids"),
...
fn write_dlc_pack_lib(
    ...
    src.push_str(
        "    pub navmesh_lookup: unsafe extern \"C\" fn(u64, *mut *const u8, *mut usize) -> bool,\n",
    );
    src.push_str(
    ...
    src.push_str(
        "#[unsafe(no_mangle)]\n/// # Safety\n/// Output pointers must be non-null, writable, and valid for this call.\npub unsafe extern \"C\" fn perro_dlc_pack_lookup_navmesh(path_hash: u64, data_out: *mut *const u8, len_out: *mut usize) -> bool {\n    // SAFETY: Caller contract supplies valid output pointers.\n    unsafe { write_bytes_out(static_assets::navmeshes::lookup_navmesh(path_hash), data_out, len_out) }\n}\n\n",
    );
    src.push_str(

...
fn write_steam_manifest(
        ...
        for ch in value.chars() {
            match ch {
                ...
                '\u{0}'..='\u{1f}' | '\u{7f}' => {
                    ...
                    write!(encoded, "\\u{:04X}", ch as u32).expect("write to String");
                }
                _ => encoded.push(ch),
            }
        }
    ...
    let manifest = format!(
        "# Generated by Perro Engine\n\n\
[engine]\n\
name = \"Perro Engine\"\n\
version = \"1.0.0\" # Hardcoded engine version\n\
website = \"https://perroengine.com\"\n\n\
[project]\n\
name = {}\n\
description = {}\n\
company = {}\n\
...
trademark = {}\n",
        ...
        toml_string(metadata.description.as_deref().unwrap_or_default()),
        toml_string(metadata.company.as_deref().unwrap_or_default()),
        toml_string(metadata.version.as_deref().unwrap_or_default()),
        toml_string(metadata.copyright.as_deref().unwrap_or_default()),
        toml_string(metadata.trademark.as_deref().unwrap_or_default()),
    );
    fs::write(output_dir.join("steam.manifest.perro"), manifest)?;
    Ok(())
}

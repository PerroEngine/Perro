...
impl PipelineRegistry {
    ...
    #[cfg(test)]
    pub(crate) fn base_family_count_for_test(&self) -> usize {
        [
            self.rigid_standard.get().is_some(),
            self.depth_prepass_rigid.get().is_some(),
            self.shadow_depth_rigid.get().is_some(),
            self.sky.get().is_some(),
            self.multimesh.get().is_some(),
            self.multimesh_depth_prepass.get().is_some(),
            self.multimesh_shadow_depth.get().is_some(),
        ]
        .into_iter()
        .filter(|ready| *ready)
        .count()

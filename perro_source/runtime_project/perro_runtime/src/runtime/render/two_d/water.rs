...
impl Runtime {
    pub(crate) fn collect_water_coastline_shapes_2d(
        ...
        let body_ids = std::mem::take(&mut self.physics_sync.water_collision_body_ids_2d_cache);
        ...
        self.physics_sync.water_collision_body_ids_2d_cache = body_ids;
        if shapes.is_empty() {
            empty_arc_slice()
        } else {
            Arc::from(shapes)
    ...
    pub(crate) fn collect_water_queries_2d(
        ...
        let Some(queries) = self.physics_sync.pending_water_queries_2d.get(&water_id) else {
            return empty_arc_slice();
        };
    ...
    pub(crate) fn collect_water_impacts_2d(
        ...
        for impact in self.physics_sync.force_water_impacts_2d.iter() {
        ...
        if let Some(contacts) = self.physics_sync.water_contacts_2d.get(&water_id) {
    ...
    pub(crate) fn collect_water_links_2d(
        ...
        let other_ids = std::mem::take(&mut self.physics_sync.water_ids_2d_cache);
        ...
        self.physics_sync.water_ids_2d_cache = other_ids;
        if links.is_empty() {
            empty_arc_slice()
        } else {
            Arc::from(links)

...
fn apply_audio_zone_2d_data(node: &mut AudioZone2D, data: &SceneDefNodeData) {
...
fn apply_audio_zone_3d_data(node: &mut AudioZone3D, data: &SceneDefNodeData) {
    for (name, value) in flatten_scene_node_fields(data) {
        match name.as_ref() {
            "enabled" => {
                if let Some(v) = as_bool(&value) {
...
fn apply_audio_zone_effect(effect: &mut perro_nodes::AudioZoneEffect, value: &SceneValue) {

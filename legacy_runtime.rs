...
pub fn convert_flamegraph(folded_path: &Path, svg_path: &Path) {
            ...
            match File::create(svg_path) {
                Ok(svg_file) => {
                    ...
                    match flamegraph::from_reader(&mut options, reader, writer) {
                        ...
                        Err(e) => {
                            eprintln!("??  Failed to convert with inferno: {}", e);
                        }
                    }
                }
                Err(e) => {
                    eprintln!("??  Failed to create SVG file: {}", e);
                }
            }
        }
        Err(e) => {
            eprintln!("??  Failed to open folded file: {}", e);
        }
    }
    ...
    match output {
        ...
        Ok(_) => {
            eprintln!("??  flamegraph command failed. Install with: cargo install flamegraph");
            eprintln!(
        ...
        Err(_) => {
            eprintln!("??  flamegraph command not found. Install with: cargo install flamegraph");
            eprintln!(
                "   Or manually convert: flamegraph {:?} > {:?}",
                folded_path, svg_path
            );
...
pub fn run_dev_with_path(project_root: PathBuf) {
    ...
    if enable_profiling {
        eprintln!("??  Profiling requested but not enabled!");
        eprintln!(

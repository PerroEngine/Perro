...
impl<'a> ScriptApi<'a> {
    ...
    {
        // Create the node - Default::default() will call Node::new() which generates an ID
        let node: T = Default::default();
        let mut scene_node = node.to_scene_node();
        
        // In DLL mode (debug): Replace ID with arena-based one (static counter might collide across DLLs)
        // In release mode: Use the ID already generated by Node::new() -> NodeID::new() (no need to re-read it)
        #[cfg(debug_assertions)]
        {
            // DLL mode: Get next ID from arena and set it
            let arena_id = self.scene.next_node_id();
            scene_node.set_id(arena_id);
        }
        // In release mode, the ID is already set by Default::default() -> Node::new() -> NodeID::new()
        // No need to do anything - just use it as-is

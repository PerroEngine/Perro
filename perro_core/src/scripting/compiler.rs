...
impl Compiler {
    ...
    fn codegen_scenes_file(&self, static_assets_dir: &Path) -> anyhow::Result<()> {
        ...
        while let Some(current_res_path) = scene_queue.pop_front() {
            ...
            let key_to_uuid: std::collections::HashMap<u32, crate::uid32::Uid32> = scene_data.key_to_node_id()
                .iter()
                .map(|(&key, node_id)| (key, node_id.as_uid32()))
                .collect();
            let mut new_key_to_uuid: std::collections::HashMap<u32, crate::uid32::Uid32> = std::collections::HashMap::new();
            
            // First pass: generate deterministic Uid32s for all nodes
            for (&key, node) in scene_data.nodes.iter_mut() {
                // Generate deterministic Uid32 from key (use key as u32 directly, or hash it)
                use crate::uid32::{Uid32, NodeID};
                let uid = Uid32::from_u32(key);
                node.set_id(NodeID::from_uid32(uid));
                new_key_to_uuid.insert(key, uid);
            }
            ...
            for node in scene_data.nodes.values_mut() {
                ...
                if let Some(parent) = node.get_parent() {
                    ...
                    let parent_key_opt = key_to_uuid.iter()
                        .find(|&(_, &uuid)| uuid == parent.id.as_uid32())
                        .map(|(&key, _)| key);
                    
                    if let Some(parent_key) = parent_key_opt {
                        if let Some(&parent_uuid) = new_key_to_uuid.get(&parent_key) {
                            use crate::uid32::NodeID;
                            let parent_type = crate::nodes::node::ParentType::new(NodeID::from_uid32(parent_uuid), parent.node_type);
                ...
                for child_id in children {
                    ...
                    let child_key_opt = key_to_uuid.iter()
                        .find(|&(_, &uuid)| uuid == child_id.into())
                        .map(|(&key, _)| key);
                    
                    if let Some(child_key) = child_key_opt {
                        if let Some(&child_uuid) = new_key_to_uuid.get(&child_key) {
                            let child_node_id = NodeID::from_uid32(child_uuid);
                            // Only add if we haven't seen this NodeID before
                            if seen_children.insert(child_node_id) {
...
",
    ...
    fn codegen_fur_file(&self, static_assets_dir: &Path) -> anyhow::Result<()> {
        ...
        writeln!(fur_file, "use perro_core::Uid32;")?;

...
impl Compiler {
    ...
    fn codegen_scenes_file(&self, project_src_dir: &Path) -> anyhow::Result<()> {
        ...
        writeln!(scenes_file, "use uuid::Uuid;")?;
        ...
        writeln!(scenes_file, "use std::collections::HashMap;")?;
        ...
        while let Some(current_res_path) = scene_queue.pop_front() {
            ...
            let file_content = fs::read_to_string(&full_fs_path)?;
            let mut scene_data: SceneData = serde_json::from_str(&file_content)?;
            ...
            for (uuid, node) in &scene_data.nodes {
                ...
                node_str = uuid_literal_regex
                    .replace_all(&node_str, "Uuid::parse_str(\"$1\").unwrap()")
                    .to_string();
                ...
                node_str = string_some_regex
                    .replace_all(&node_str, "Some(\"$1\".to_string())")
                    .to_string();

                let string_field_regex = Regex::new(r#":\s*"([^"]+)","#)?;
                node_str = string_field_regex
                    .replace_all(&node_str, ": \"$1\".to_string(),")
                    .to_string();
                ...
                if let Some(open_paren) = node_str.find('(') {
                    if let Some(variant_pos) = node_str.find("SceneNode::") {
                        ...
                        writeln!(
                            &mut entries,
                            "        (Uuid::parse_str(\"{}\").unwrap(), SceneNode::{}({})),",
...
static {name}: Lazy<SceneData> = Lazy::new(|| SceneData {{
    root_id: Uuid::parse_str(\"{root_id}\").unwrap(),
    nodes: {nodes},
}});
",
    ...
    if path.is_file() && path.extension().map_or(false, |ext| ext == "fur") {
        println!("cargo:rerun-if-changed={}", path.display());
        let relative_path = path.strip_prefix(&res_dir)?.to_string_lossy().to_string();
        let res_path = format!("res://{}", relative_path.replace('\\', "/")); // ✅ FIXuse perro_core::ui::ast::{FurElement, FurNode};
...
"#,
    ...
    fn codegen_fur_element(&self, element: &FurElement, indent_level: usize) -> anyhow::Result<String> {
        ...
        writeln!(&mut code, "{}    tag_name: \"{}\".to_string(),", indent, element.tag_name)?;
        writeln!(&mut code, "{}    id: \"{}\".to_string(),", indent, element.id)?;
        ...
        if element.attributes.is_empty() {
            ...
            for (key, value) in &element.attributes {
                writeln!(&mut code, "{}        (\"{}\".to_string(), \"{}\".to_string()),", indent, key, value.replace("\"", "\\\""))?;
            }
        ...
        if element.children.is_empty() {
            ...
            for child in &element.children {
                match child {
                    ...
                    FurNode::Text(text) => {
                        writeln!(&mut code, "{}        FurNode::Text(\"{}\".to_string()),", indent, text.replace("\"", "\\\""))?;
                    }
                }
            }
            writeln!(&mut code, "{}    ],", indent)?;
...
fn codegen_manifest_file(&self, project_src_dir: &Path) -> anyhow::Result<()> {
    ...
    writeln!(manifest_file, "use std::collections::HashMap;")?;
    ...
    // Create a new Project with all the settings
    writeln!(manifest_file, "    let mut project = Project::new_static(")?;
    ...
    if !project.metadata().is_empty() {
        ...
        for (key, value) in project.metadata() {
            writeln!(
                manifest_file,
                "    project.set_meta(\"{}\".to_string(), \"{}\".to_string());",
                key,
                value.replace("\"", "\\\"")
            )?;
        }

...
use crate::scripting::ast::{Expr, Stmt, Type, Variable};
...
fn generate_batched_dynamic_mutation(
    ...
    let all_base_node_fields = field_paths.iter().all(|path| {
        path.len() == 1
            && ENGINE_REGISTRY
                .get_field_type_node(&NodeType::Node, &path[0])
                .is_some()
    });
    ...
    if all_base_node_fields && field_paths.len() == 1 {
        ...
        if let Some(setter) = setter_method {
            ...
            for (_, rhs_expr) in &mutation_data {
                ...
                let expected_setter_type =
                    ENGINE_REGISTRY.get_field_type_node(&NodeType::Node, first_field);

                // Generate RHS code with the expected type as a hint
                let rhs_code = rhs_expr.expr.to_rust(
                    needs_self,
                    script,
                    expected_setter_type.as_ref(),
                    current_func,
    ...
    if compatible_node_types.is_empty() {
        ...
        for (_lhs_expr, stmt) in mutations {
    ...
    let clean_closure_var = if let Some((_, _, _, closure_var)) =
        extract_node_member_info(&mutations[0].0.expr, script, current_func)
    {
        closure_var
            .strip_prefix("self.")
    ...
    if compatible_node_types.len() == 1 {
        ...
        for (field_path_vec, rhs_expr) in &mutation_data {
            let modified_rhs = extract_api_calls_simple(
                &rhs_expr.expr,
                script,
                current_func,
                &mut all_extracted_api_calls,
                &mut temp_var_types,
                &mut temp_counter,
            );
            let rhs_code = modified_rhs.to_rust(
                needs_self,
                script,
                None,
                current_func,
            ...
            batched_body.push(format!(
                "{}.{} = {};",
                clean_closure_var, resolved_field_path, rhs_code
            ));
        }

        let temp_decl = if !all_extracted_api_calls.is_empty() {
            Some(
                all_extracted_api_calls
                    .iter()
...
impl Function {
    pub fn to_rust_method(&self, _node_type: &str, script: &Script) -> String {
        ...
        while i < self.body.len() {
            ...
            if let Stmt::MemberAssign(lhs_expr, _) = stmt {
                if let Some((node_id, node_type, _, _)) =
                    extract_node_member_info(&lhs_expr.expr, script, Some(self))
                {
                    if node_type == "__DYN_NODE__" {
                        ...
                        while j < self.body.len() {
                            if let Stmt::MemberAssign(next_lhs, _) = &self.body[j] {
                                if let Some((next_node_id, next_node_type, _, _)) =
                                    extract_node_member_info(&next_lhs.expr, script, Some(self))
                                {
                        ...
                        if mutations.len() > 1 {
                            ...
                            out.push_str(&generate_batched_dynamic_mutation(
                                &mutations,
                                node_id,
                                needs_self,
                                script,
    ...
    pub fn to_rust_free_function(
        ...
        write!(
            out,
            "pub fn {}({}) -> {} {{\n",
            rust_func_name, param_list, return_type
        )
        .unwrap();

...
// Include the compile-time timestamp constant generated by build.rs
include!(concat!(env!("OUT_DIR"), "/compile_time_timestamp.rs"));
...
pub fn compute_script_hash(project_root: &Path) -> Result<String, String> {
    ...
    // Include compile-time unix timestamp in the hash
    // This ensures the hash changes when perro_core is recompiled,
    // even if the script files themselves haven't changed
    hasher.update(b"COMPILE_TIME_TIMESTAMP:");
    hasher.update(COMPILE_TIME_UNIX_TIMESTAMP.to_string().as_bytes());
    hasher.update(b"\0");

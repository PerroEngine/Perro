...
Use `Ui*` nodes for persistent, interactive screen structure. Use `Draw2D` for transient shapes redrawn each frame that need no focus, layout, or input state. Let gameplay own the source value and send the UI a method call or signal; do not make a label query the world each frame when the owner already knows the value changed.

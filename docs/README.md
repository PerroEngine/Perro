...
- **Dev mode:** The scripts crate is compiled as a **DLL**. The engine (e.g. `perro_dev`) loads that DLL at runtime. When you change a script, only the scripts crate is recompiled, so iteration is fast. You never run PUP/TS/C# — you run the compiled Rust in that DLL.

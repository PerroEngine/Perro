...
pub fn get_completions(document: &ParsedDocument, position: Position) -> Vec<CompletionItem> {
    match document {
        ParsedDocument::Pup { script, source, .. } => get_pup_completions(script, source, position),
        ParsedDocument::Fur { source, .. } => get_fur_completions(source, position),
    }
}
...
fn get_pup_completions(script: &Script, source: &str, position: Position) -> Vec<CompletionItem> {
    ...
    let text_to_check =
        if char_offset < full_line.len() && full_line.chars().nth(char_offset) == Some('.') {
    ...
    if char_offset > 0
        && char_offset <= full_line.len()
        && full_line.chars().nth(char_offset - 1) == Some('.')
    {
    ...
    for var in &script.variables {
        items.push(create_completion_item(
            ...
            var.typ
                .as_ref()
                .map(|t| format!("Type: {}", type_to_string(t))),
        ));
    }
...
fn get_api_module_completions(module_name: &str) -> Vec<CompletionItem> {
    ...
    for method_name in method_names {
        ...
        let (return_type, param_types, param_names) = if let Some(api_module) =
            PupAPI::resolve(module_name, method_name)
        {
...
fn get_self_completions(script: &Script) -> Vec<CompletionItem> {
    ...
    if let Some(node_type) = ENGINE_REGISTRY
        .node_defs
        .keys()
        .find(|nt| nt.type_name() == node_type_str)
    {
    ...
    if let Some(node_type) = ENGINE_REGISTRY
        .node_defs
        .keys()
        .find(|nt| format!("{:?}", nt) == *node_type_str)
    {
        return get_node_completions(*node_type);
    }
...
fn fix_type_for_completion(typ: &Type) -> Type {
    match typ {
        Type::Container(kind, types) => {
            let fixed_types: Vec<Type> = types
                .iter()
                .map(|t| match t {
